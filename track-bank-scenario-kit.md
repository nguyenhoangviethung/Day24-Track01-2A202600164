# Bộ track scenario kit Day 24

File này cung cấp bối cảnh sản phẩm đủ rõ để bạn chọn track và tự suy ra scenario, failure mode, test set, eval plan.

---

## Nguyên tắc thiết kế track

Track không phải answer key. Track chỉ đưa học viên vào một bối cảnh sản phẩm cụ thể.

Track packet **cung cấp**:

```text
1. Tên track
2. Bối cảnh sản phẩm
3. Điểm chạm với AI
4. Flow người dùng mẫu
```

Học viên **tự điền**:

```text
1. Kỳ vọng người dùng
2. Scenario phát sinh từ kỳ vọng
3. AI nên được phép làm gì
4. AI không nên được làm gì
5. Nguồn thông tin cần có
6. Failure mode
7. Safety question
8. Test set v0
9. Eval plan v0
...
Không đưa sẵn failure mode, test set, eval plan, hoặc solution trong track packet. Những phần đó là bài tập tư duy của học viên.

---

## Worksheet dùng chung cho mọi track

Sau khi chọn track, học viên điền theo thứ tự này:
...
```text
- User đang muốn hoàn thành việc gì?
- User kỳ vọng AI biết hoặc làm được điều gì?
- User có thể hiểu nhầm AI là kênh chính thức ở điểm nào?
- Nếu tin câu trả lời sai của AI, user có thể hành động sai ra sao?
- Ai khác bị ảnh hưởng ngoài người trực tiếp dùng AI?
```

### Tình huống phát sinh từ kỳ vọng

Học viên tự viết 3-5 scenario theo template:

```text
User là [ai], đang ở [điểm chạm], muốn [mục tiêu],
nên hỏi/nhờ AI: "[câu hỏi hoặc yêu cầu]".
Nếu AI xử lý sai, hậu quả có thể là [hậu quả].
```

### AI nên được phép làm gì

Gợi ý:

```text
- AI nên tra cứu, tóm tắt, giải thích, gợi ý, hay được hành động?
- Phần nào AI có thể tự trả lời?
- Phần nào AI chỉ nên draft để người thật duyệt?
- Phần nào cần chuyển sang người thật?
```

### AI không nên được làm gì

Gợi ý:

```text
- AI không được hứa/cam kết/quyết định điều gì?
- AI không được suy đoán điều gì khi thiếu dữ liệu?
- Case nào phải từ chối?
- Case nào phải escalate?
```

### Nguồn thông tin cần có

Gợi ý:

```text
- Câu trả lời đúng phải dựa vào tài liệu/dữ liệu nào?
- Nguồn nào là nguồn chính thức?
- Nếu nguồn không có thông tin thì AI nên làm gì?
- Nguồn nào có thể lỗi thời, mâu thuẫn, hoặc không đủ?
```

### Dạng lỗi, test set, eval plan, solution design

Học viên tiếp tục tự xác định:

```text
- Primary failure mode:
- Safety question:
- Test set v0: 5 case
- Eval plan v0: pass/fail/unclear + severity rule
- Solution design: chọn 1-2 layer phù hợp
```
...
## Menu lớp giải pháp cho Day 25

Sau khi học viên tự xác định failure case, nhóm có thể chọn 1-2 layer để thiết kế solution ở Day 25. Phần này chỉ là gợi ý, không phải bài nộp Day 24.

| Layer | Gợi ý artifact Day 25 | Phù hợp khi |
|---|---|---|
